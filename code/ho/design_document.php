...
function init(){
      ...
      $designDocs[] = array(
        ...
        'updates' => array(
          "store_shift"=>"function(doc,req){ if(doc) { var data = new Object(); if(req.query.type == 'day_end') { doc.day.petty_cash_balance.opening_petty_cash = req.query.opening_petty_cash; doc.day.petty_cash_balance.petty_expense = req.query.petty_expense; doc.day.petty_cash_balance.closing_petty_cash = req.query.closing_petty_cash; doc.day.petty_cash_balance.inward_petty_cash = req.query.inward_petty_cash; doc.day.end_time = req.query.time; doc.day.end_fullcash = req.query.cash; doc.day.end_login_id = req.query.login; doc.day.end_staff_name = req.query.name } else if(req.query.type == 'shift_start') {data.shift_no = doc.shift.length+1;  doc.shift.push({start_time:req.query.time, start_login_id:req.query.login, start_staff_name:req.query.name, counter_no:req.query.counter_no,shift_no:data.shift_no, end_staff_name:'', end_time:'',petty_cash_balance:{opening_petty_cash:'',petty_expense:'',closing_petty_cash:'',inward_petty_cash:''}}); }else if(req.query.type == 'shift_end') { req.query.shift_no = doc.shift.length-1; doc.shift[req.query.shift_no].end_time = req.query.time; doc.shift[req.query.shift_no].end_petty_cash = req.query.end_petty_cash; doc.shift[req.query.shift_no].end_cash_inbox = req.query.box_cash;doc.shift[req.query.shift_no].end_login_id = req.query.login;doc.shift[req.query.shift_no].end_staff_name = req.query.name; doc.shift[req.query.shift_no].petty_cash_balance.opening_petty_cash = req.query.opening_petty_cash; doc.shift[req.query.shift_no].petty_cash_balance.petty_expense = req.query.petty_expense; doc.shift[req.query.shift_no].petty_cash_balance.closing_petty_cash = req.query.closing_petty_cash; doc.shift[req.query.shift_no].petty_cash_balance.inward_petty_cash = req.query.inward_petty_cash;}  return [doc,JSON.stringify(data)];}}"
        )
      );
      ...
      $designDocs[] = array(
        "_id" => "_design/petty_expense",
          ...
          "views" => array(
              ...
              "get_inward" => array(
                "map" => "function(doc) { if(doc.cd_doc_type && doc.cd_doc_type=='petty_inward'){ emit(doc.date, doc.amount); } }"
                )
            )
      );
