...
//			http://54.249.247.15:5984/rakesh_cpos_ho/_design/staff/_view/staff_location?key=%222%22
		...
		function init(){
			...
			$designDocs[] = array(
				...
				'updates' => array( 
					'getbillno' =>  "function(doc,req){ if(req.query.store_id) { if(req.query.store_id in doc.bill_array ) { if(doc.bill_array[req.query.store_id]['date']==req.query.date && doc.bill_array[req.query.store_id]['counter']==req.query.counter1){doc.bill_array[req.query.store_id]['bill_no'] = doc.bill_array[req.query.store_id]['bill_no']+1;}else{doc.bill_array[req.query.store_id]['date'] = req.query.date;doc.bill_array[req.query.store_id]['counter'] = req.query.counter1;doc.bill_array[req.query.store_id]['bill_no'] = 1;}}else{doc.bill_array[req.query.store_id] = new Object();doc.bill_array[req.query.store_id]['date'] = req.query.date;doc.bill_array[req.query.store_id]['counter'] = req.query.counter1;doc.bill_array[req.query.store_id]['bill_no'] = 1} var newCurrent =  doc.bill_array[req.query.store_id]['bill_no']; doc.current = newCurrent; return [doc,newCurrent.toString()];}}",
				),
				'lists' => array( 
					...
					'caw_sale' => "function(head,req){var billList = new Object(); while(row = getRow()){ if(row.doc) { if(row.doc.payment_type=='caw'){if(row.doc.customer.id in billList){}else{billList[row.doc.customer.id] = new Object;} billList[row.doc.customer.id][row.doc.customer.onsite_time] = row.doc.customer.challan_no; }}} return JSON.stringify(billList);}"
			...
			$designDocs[] = array(
				...
				'updates' => array(
					"store_shift"=>"function(doc,req){ if(doc) { var data = new Object(); if(req.query.type == 'day_end') { doc.day.petty_cash_balance.petty_expense = req.query.petty_expense; doc.day.petty_cash_balance.closing_petty_cash = req.query.closing_petty_cash; doc.day.petty_cash_balance.inward_petty_cash = req.query.inward_petty_cash; doc.day.end_time = req.query.time; doc.day.end_fullcash = req.query.cash; doc.day.end_login_id = req.query.login; doc.day.end_staff_name = req.query.name } else if(req.query.type == 'shift_start') {data.shift_no = doc.shift.length+1;  doc.shift.push({end_petty_cash:'',opening_cash_inbox:req.query.opening_box_cash,end_cash_inbox:'',start_time:req.query.time, start_login_id:req.query.login, start_staff_name:req.query.name, counter_no:req.query.counter_no,shift_no:data.shift_no, end_staff_name:'', end_time:'',petty_cash_balance:{opening_petty_cash:req.query.opening_petty_cash,petty_expense:'',closing_petty_cash:'',inward_petty_cash:''},cash_denomination:{qty_10:req.query.qty_10,qty_20:req.query.qty_20,qty_50:req.query.qty_50,qty_100:req.query.qty_100,qty_500:req.query.qty_500,qty_1000:req.query.qty_1000,qty_sodex:req.query.qty_sodex,qty_ticket_res:req.query.qty_restaurent}}); }else if(req.query.type == 'shift_end') { req.query.shift_no = doc.shift.length-1; doc.shift[req.query.shift_no].end_time = req.query.time; doc.shift[req.query.shift_no].end_petty_cash = req.query.end_petty_cash; doc.shift[req.query.shift_no].end_cash_inbox = req.query.box_cash;doc.shift[req.query.shift_no].end_login_id = req.query.login;doc.shift[req.query.shift_no].end_staff_name = req.query.name;  doc.shift[req.query.shift_no].petty_cash_balance.petty_expense = req.query.petty_expense; doc.shift[req.query.shift_no].petty_cash_balance.closing_petty_cash = req.query.closing_petty_cash; doc.shift[req.query.shift_no].petty_cash_balance.inward_petty_cash = req.query.inward_petty_cash;doc.shift[req.query.shift_no].cash_denomination.qty_10 = req.query.qty_10;doc.shift[req.query.shift_no].cash_denomination.qty_20 = req.query.qty_20;doc.shift[req.query.shift_no].cash_denomination.qty_50 = req.query.qty_50;doc.shift[req.query.shift_no].cash_denomination.qty_100 = req.query.qty_100;doc.shift[req.query.shift_no].cash_denomination.qty_500 = req.query.qty_500;doc.shift[req.query.shift_no].cash_denomination.qty_1000 = req.query.qty_1000;doc.shift[req.query.shift_no].cash_denomination.qty_sodex = req.query.qty_sodex;doc.shift[req.query.shift_no].cash_denomination.qty_ticket_res = req.query.qty_restaurent;} else if(req.query.type == 'cash_reconciliation') {delete req.query.type; doc.day.cash_reconciliation = new Object(); for (var name in req.query) { doc.day.cash_reconciliation[name] = req.query[name]; }} return [doc,JSON.stringify(data)];}}"
				)
			);
