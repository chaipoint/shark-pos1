		   ...
		   "updates": {
		      ...
		      $designDocs[] = array(
		        "_id" => "_design/design_ho",
		          ...
		          "lists" => array(
		              "sales_register" => "function(head, req) { var billList=new Object;billList.data=new Object;var payment_type=new Object;payment_type.count=new Object;payment_type.amount=new Object;var bill_status=new Object;bill_status.count=new Object;bill_status.amount=new Object;var cashSale=0; var creditSale = 0; var ppaSale = 0; var ppcSale=0;var cashinDelivery=0;var store_bill_list=new Object;while(row=getRow()){if(row.doc){if(!(row.doc.store_id in billList.data)){billList.data[row.doc.store_id]=new Object}if(!(row.doc.bill_no in billList.data[row.doc.store_id])){billList.data[row.doc.store_id][row.doc.bill_no]=new Object;if(row.doc.bill_status_id==68&&row.doc.payment_type=='cash'){cashSale+=1*row.doc.due_amount}if(row.doc.bill_status_id==68&&row.doc.payment_type=='ppc'){ppcSale+=1*row.doc.due_amount} if(row.doc.bill_status == 'Paid' && row.doc.payment_type == 'ppa') { ppaSale += (1* row.doc.due_amount); } if(row.doc.is_credit == 'Y' && row.doc.payment_type == 'credit') { creditSale += (1* row.doc.due_amount); }   if(row.doc.bill_status_id==65&&row.doc.bill_status=='CoD'){cashinDelivery+=1*row.doc.due_amount}}}}billList.payment_type=payment_type;billList.cash_sale=cashSale;billList.ppcSale=ppcSale; billList.ppaSale = ppaSale; billList.creditSale = creditSale;  billList.cashinDelivery=cashinDelivery;delete billList.data; return JSON.stringify(billList); }",
